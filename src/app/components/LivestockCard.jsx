...
export default function LivestockCard({ listing }) {
  // Balanced Status Badge Styles mapped cleanly to brand system values
  ...
  const whatsappNumber = "266XXXXXXXX"; 
  ...
  return (
    // Crisp white floating card frame isolated clearly over the F8F6F2 base canvas
    <div className="bg-white rounded-[2rem] overflow-hidden shadow-xs hover:shadow-lg transition-all duration-300 border border-[#E5E7EB] flex flex-col w-full max-w-sm">
      ...
      {/* Image Carousel (Uses the yellow accent color configuration inside) */}
      ...
      <div className="p-6 flex-grow flex flex-col justify-between">
        <div>
          ...
          {/* Title - Exact #20352E Specification */}
          <h3 className="text-xl font-bold text-[#20352E] tracking-tight mt-1">
            {listing.title}
          </h3>

          {/* Location pin with #6D8077 Secondary Tone */}
        ...
        {/* Action Button Layer - Styled with Primary Action Green #3D7A5E */}
        <div className="mt-6">
          <a
            href={whatsappUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="w-full bg-[#3D7A5E] hover:bg-[#285F44] text-white py-3.5 px-4 rounded-xl font-bold text-sm tracking-wide transition-colors shadow-xs flex items-center justify-center space-x-2"
          >
